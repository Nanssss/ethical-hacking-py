...
def process_packet(packet):
	...
	if scapy_packet.haslayer(scapy.DNSRR): #si c'est une reponse DNS ; sinon DNSQR pour request
		...
		if "horizons21.fr" in str(qname): #si le site est dans le qname
			print("trouve")
...
os.system(f"iptables -A FORWARD -m state --state ESTABLISHED,RELATED -i eth0 -j ACCEPT")
#os.system('sudo iptables -t nat PREROUTING -p udp --dport 53 -j NFQUEUE --queue-num 0')
